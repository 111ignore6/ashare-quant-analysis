...
Write-Output "== 4/4 启动仪表盘（前台运行：Ctrl+C 或关闭本窗口即停止）=="
...
if ($existing) {
    ...
    Write-Output "仪表盘启动中：http://localhost:8501（Ctrl+C 或关闭窗口即停止仪表盘）"
    python -X utf8 -u -m streamlit run dashboard.py --server.port 8501
}
Write-Output "一条龙完成。"

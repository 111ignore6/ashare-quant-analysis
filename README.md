...
**这个仓库由 [DSH（DeepSeek Harness）](https://github.com/deepseek-ai/deepseek-harness)
—— 一个 AI 编码 agent 运行时 —— 持续开发与维护**，人类机主负责定方向与验收。
它不是人类手写的作品集。

这意味着几件事，请在阅读代码与结论前知悉：

- **代码、文档、提交信息、以及绝大多数"审查结论"都出自 DSH 会话**，
  人类机主负责方向、验收与对外发布决定；
- **`AGENTS.md` 是 DSH 的项目级长期记忆**（工程日志）：每次新开会话操作本仓库前
  先读它，避免重复踩坑。它记录的是"踩过什么坑、哪条结论被后来的证据推翻了"
  —— 写法刻意保留原始证据强度，而不是成功叙事；
- **DSH 会犯错，本项目选择把错误留在记录里而不是删掉**。文中的
  「未证实」「已推翻」「探针自己说谎」等字样都是真实发生过的事，
  详见 [`docs/HONESTY.md`](docs/HONESTY.md)；
- 因此**请对任何"漂亮"的数字保持默认怀疑**，并按 `docs/HONESTY.md`
  第八节的引用规范核对口径；
- **用编码 agent 操作本仓库时，请先让它读 `AGENTS.md`** —— 里面记录了绝大多数
  已经踩过的坑（复权锚点、成交量量纲、缓存判据、计划任务编码……）。
